...
void SimpleLoggerMgr::handleStackTrace(int sig, siginfo_t* info, void* secret) {
  ...
  kill(mgr->crashOriginThread, SIGUSR2);
#endif
}
#endif
...
void SimpleLogger::put(int level, const char* source_file, const char* func_name, size_t line_number, const char* format, ...) {
  ...
  avail_len = MSG_SIZE;
#ifdef __linux__
  _snprintf(msg, avail_len, cur_len, msg_len, " [" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") "." _CL_BROWN("%03d") " " _CL_BROWN("%03d") "] [tid " _CL_B_BLUE("%*u")
                                                  "] "
                                                  "[%s] ",
            lt.hour,
            lt.min,
...
#else
  _snprintf(msg, avail_len, cur_len, msg_len, " [" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") ":" _CL_BROWN("%02d") "." _CL_BROWN("%03d") " " _CL_BROWN("%03d") "] [tid " _CL_B_BLUE("%04x")
                                                  "] "
                                                  "[%s] ",
            lt.hour,
            lt.min,
